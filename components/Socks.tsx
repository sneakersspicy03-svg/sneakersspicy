...
interface SocksProps {
  ...
  isDevMode?: boolean;
  // Fix: Added missing onSelectSize prop to interface
...
const Socks: React.FC<SocksProps> = ({ brands, onBrandSelect, onQuickAdd, isDevMode = false, onSelectSize }) => {
  return (
                ...
                <div className="absolute inset-0 bg-black/85 backdrop-blur-xl transition-all duration-700 flex flex-col items-center justify-center p-8 opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100 z-50">
                  <div className="text-center space-y-8 w-full max-w-[240px]">
                    ...
                    <div className="flex flex-col space-y-3 w-full">
                      {/* Fix: Implement size grid in UI using onSelectSize to provide consistent functionality */}
                      {onSelectSize && brand.availableSizes && brand.availableSizes.length > 0 && (
