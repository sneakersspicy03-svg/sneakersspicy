...
export const StockXProductCard: React.FC<StockXProductCardProps> = ({
  ...
  return (
    <div
      onClick={onClick}
      className="group relative flex flex-col bg-[#0D0D0F] rounded-2xl border border-white/10 hover:border-red-600/60 transition-all duration-300 cursor-pointer overflow-hidden p-2 sm:p-3 hover:shadow-2xl hover:shadow-red-950/20 select-none w-full active:scale-[0.98]"
    >
      {/* Product Image Container */}
      <div className="relative aspect-square w-full flex items-center justify-center bg-zinc-900/60 rounded-xl p-0.5 overflow-hidden mb-2 border border-white/5">
        <img
          src={product.image}
          alt={product.name}
          loading="lazy"
